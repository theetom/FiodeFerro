...
/*   Updated: 2024/09/03 15:37:49 by toferrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void x_line(int n, double xx, double yy, t_data *data)
{
	double		xx1;
	double		yy1;
	double		a;
	...
	while (x < xx1)
	{
		...
		b = yy - (a * xx);
		c = (a * x + b);
		if (x > 0 && x < data->img_w && c < data->img_h && 0 < c)
			my_mlx_pixel_put(data, x, c, 0xFFFFFF);
		x += 0.01;
	}
}

void y_line(int n, double xx, double yy, t_data *data)
{
	double		xx1;
	double		yy1;
	double		a;
	...
	{ 
		...
		if (x > 0 && x < 1920 && c < 1080 && 0 < c)
			my_mlx_pixel_put(data, x, c, 0x00FFFFFF);
		x += 0.01;
	}
}
...
void two_to_three(t_data *data)
{
	...
	while(data->tdp[n])
	{
		xx = (data->position_w + data->scale * ((data->tdp[n][0]) * cos(120) +
				(data->tdp[n][1]) * cos(120 + 2) + (data->tdp[n][2]) * cos(120 - 2)));
		yy = (data->position_h + data->scale * ((data->tdp[n][0]) * sin(120) +
